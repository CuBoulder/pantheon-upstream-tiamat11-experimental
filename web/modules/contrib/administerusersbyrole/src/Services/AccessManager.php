...
class AccessManager implements AccessManagerInterface {
  ...
  /**
   * The configuration for the module settings.
   *
  ...
  public function managedRoles() {
    // Filter out roles that have the 'administer users' permission.
    $roles = array_filter(Role::loadMultiple(), function ($role) {
      return !$role->hasPermission('administer users');
    });

    // Unset the anonymous and authenticated role.
  ...
  /**
   * Initial access check for an operation to test if access might be granted.
   *
  ...
  protected function preAccess(&$operation, AccountInterface $account) {
    // Full admins already have permissions so we are wasting time to continue.
